...
def fast_preview(frame, maxv, out_width=960):
    """2x2 RGGB bin -> WB -> Reinhard tonemap -> resize -> BGR uint8."""
    H, W = frame.shape
    He, We = (H // 2) * 2, (W // 2) * 2
    f = frame[:He, :We].astype(np.float32)
    R = f[0::2, 0::2]
    Gr = f[0::2, 1::2]
    Gb = f[1::2, 0::2]
    B = f[1::2, 1::2]
    rgb = np.dstack([R, 0.5 * (Gr + Gb), B]) / maxv
